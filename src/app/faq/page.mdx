...
Use the [script](/install/linux#install-script) to install the AppImage version, which should work fine on most systems. Don't forget to follow the quickstart guide for your environment once you are done installing.
...
## How to fix: "extension manager is not running" ?

This likely means you don't have [nodejs](https://nodejs.org) installed on your system, which is required if you want to run third-party extensions.

You should install a recent enough version of node (>= 20) however you like and make sure the `node` binary is in your `PATH`.

Most of the packages distributing vicinae do list `nodejs` as an optional dependency, but maybe the particular one you used doesn't. In that case you are encouraged to reach out to the maintainer of that package.
...
<Note>
vicinae uses `top` instead of `overlay` for various reasons. On some environments, some IME popups end up being shown behind the vicinae window if it's
set to overlay.
</Note>
