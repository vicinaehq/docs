...
Just create a new shortcut that binds to e.g. `vicinae vicinae://launch/clipboard/history`.

![Create a deeplink shortcut](/kde/create-deeplink-shortcut.png)

## Blur

Since version `v0.19.7`, vicinae implements the [kwin blur wayland protocol](https://wayland.app/protocols/kde-blur#org_kde_kwin_blur) thanks to which it can request blur by itself. KDE's default blur is pretty strong so you might have to decrease opacity quite a bit in order to get the most out of it.
