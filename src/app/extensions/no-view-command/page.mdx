...
The command is unloaded when the function returns **or** if another command is launched (command execution is non-blocking).

<Warning>
No-view commands are not meant to execute long-running logic. There is currently no way for extensions to do this outside of using external daemons (not recommended).
