...
An LRU disk cache for data that is expensive to fetch. Unlike `LocalStorage`, cache entries are evicted when the capacity is exceeded and data is not encrypted.
