...
Vicinae also includes a set of extra themes that are installed but not enabled by default. To add these to your configuration (assuming Vicinae is installed under /usr/local):

```bash
mkdir -p ~/.config/vicinae/themes
cp -r /usr/local/share/vicinae/themes/* ~/.config/vicinae/themes/
```
